# Posts Page Setup Guide

## Database Setup Required

The posts page requires database tables to be created in Supabase. Follow these steps:

### Step 1: Create Posts Tables
...
### Step 2: Create Storage Bucket for Media

Run this SQL to create the storage bucket for post media:
...
2. Check the "Table Editor" to see the new tables:
   - `posts`
   - `post_likes`
   - `post_comments`
   - `post_saves`
3. Check the "Storage" section to see the `post-media` bucket
4. Refresh your application and try creating a post

### Troubleshooting

If you still get errors:

1. **404 Error**: Make sure all tables were created successfully
2. **409 Error**: Check if there are any constraint violations
3. **Permission Errors**: Verify RLS policies are set up correctly
4. **Storage Errors**: Make sure the storage bucket exists and policies are correct

### Alternative: Use the Setup Files

You can also use the provided setup files:
- `posts_database_setup.sql` - Contains all the table creation SQL
- `storage_setup.sql` - Contains the storage bucket setup

Just copy and paste the contents into your Supabase SQL Editor and run them.
