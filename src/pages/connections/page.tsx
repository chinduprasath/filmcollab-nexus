                  ...
                  <div key={c.id} className="border border-gray-200 rounded-lg p-3 bg-white flex flex-col gap-2 hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-2 cursor-pointer" onClick={() => handleOpenProfile(c.id)}>
                      ...
                      <div className="min-w-0">
                        <div className="font-medium truncate text-gray-900 text-sm">{c.name}</div>
                        <div className="text-xs text-gray-500 truncate">{c.role} • {c.location}</div>
                      </div>
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <Button size="sm" variant="outline" onClick={() => handleMessage(c.id)} className="border-gray-300 text-gray-700 hover:bg-gray-50 h-7 text-xs">Message</Button>
                        <Button size="sm" variant="destructive" onClick={() => handleRemove(c.id)} className="bg-red-600 hover:bg-red-700 text-white h-7 text-xs">Remove</Button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="bg-white rounded-lg shadow-sm p-4">
            ...
            <div className="space-y-3">
                  ...
                  .map((p) => (
                    <div key={p.id} className="border border-gray-200 rounded-lg p-3 bg-white flex flex-col gap-2 hover:shadow-md transition-shadow">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          <div className="font-medium truncate text-gray-900 text-sm">{p.name}</div>
                          <div className="text-xs text-gray-500 truncate">{p.role} • {p.location}</div>
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
              ...
              {pending.filter((p) => p.direction === pendingView).length === 0 && (
                <div className="p-6 text-center text-xs text-gray-500">No {pendingView} requests.</div>
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === "suggested" && (
          <div className="bg-white rounded-lg shadow-sm p-4">
              ...
              {suggestions.map((s) => (
                <div key={s.id} className="border border-gray-200 rounded-lg p-3 bg-white flex flex-col gap-2 hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <div className="font-medium truncate text-gray-900 text-sm">{s.name}</div>
                      <div className="text-xs text-gray-500 truncate">{s.role} • {s.location}</div>
                      {typeof s.mutuals === "number" && (
                        <div className="text-[10px] text-gray-500">{s.mutuals} mutual connections</div>
                      )}
                    </div>
                  </div>
            ...
            {suggestions.length === 0 && (
              <div className="p-6 text-center text-xs text-gray-500">No suggestions at the moment.</div>
            )}
          </div>
        )}
      </div>
